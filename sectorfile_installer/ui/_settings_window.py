...
def settings_window(root: tk.Tk) -> tk.Tk:
    ...
    label_grid_args = dict(padx=10, pady=10, ipadx=5, sticky="e")
    entry_grid_args = dict(padx=10, pady=10, ipadx=5, sticky="nsew")
    button_grid_args = dict(padx=10, pady=10)

    tk.Label(window, text=translate("language")).grid(row=0, column=0, **label_grid_args)
    ...
    selected_language_menu.grid(row=0, column=1, **entry_grid_args)

    tk.Label(window, text=translate("name")).grid(row=1, column=0, **label_grid_args)
    name_entry = tk.Entry(window)
    name_entry.grid(row=1, column=1, **entry_grid_args)
    name_entry.insert(0, settings.name)

    tk.Label(window, text=translate("vatsim_id")).grid(row=2, column=0, **label_grid_args)
    vatsim_id_entry = tk.Entry(window)
    vatsim_id_entry.grid(row=2, column=1, **entry_grid_args)
    vatsim_id_entry.insert(0, settings.vatsim_id)

    tk.Label(window, text=translate("vatsim_password")).grid(row=3, column=0, **label_grid_args)
    vatsim_password_entry = tk.Entry(window, show="*")
    vatsim_password_entry.grid(row=3, column=1, **entry_grid_args)
    vatsim_password_entry.insert(0, settings.vatsim_password)

    tk.Label(window, text=translate("rating")).grid(row=4, column=0, **label_grid_args)
    ...
    rating_menu.grid(row=4, column=1, **entry_grid_args)

    tk.Label(window, text=translate("hoppie_code")).grid(row=5, column=0, **label_grid_args)
    hoppie_code_entry = tk.Entry(window, show="*")
    hoppie_code_entry.grid(row=5, column=1, **entry_grid_args)
    hoppie_code_entry.insert(0, settings.hoppie_code)

    tk.Label(window, text=translate("euroscope_path")).grid(row=6, column=0, **label_grid_args)
    euroscope_path_entry = tk.Entry(window, width=50)
    euroscope_path_entry.grid(row=6, column=1, **entry_grid_args)
    ...
    ).grid(row=6, column=2, **button_grid_args)

    tk.Label(window, text=translate("afv_path")).grid(row=7, column=0, **label_grid_args)
    afv_path_entry = tk.Entry(window)
    afv_path_entry.grid(row=7, column=1, **entry_grid_args)
    ...
    ).grid(row=7, column=2, **button_grid_args)
    ...
    def save_settings():
        ...
        Settings.set("euroscope_path", euroscope_path_entry.get())
        Settings.save()
    ...
    ).grid(row=9, column=1, **button_grid_args)
